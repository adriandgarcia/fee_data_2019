...
Here I am looking at fund families' fees for each U.S. category types for active funds. I start by pulling in the filtered data from **ActiveFunds**.
...
write_csv(
  ...
  "shareclass_changes_activefunds.csv"
  )
```
...
Here I am looking at the fund families with the lowest and highest overall fees across passive products. I start by filtering out non-index funds and removing fees that are NA (blank) for 2020. 1,060 rows remain.
...
write_csv(
  Passive_Fees_Past,
  "past_fees_passivefunds_firm.csv"
)
...
Passive_LowFees_Past %>%
  select(1:4,
         6:7,
         5, 8) %>%
  write_csv("past_lowfees_passivefunds_firm.csv")
```
...
Passive_Cat_Pct <- Passive_Cat %>%
  ...
  write_csv(Passive_Cat_Pct, "passivefunds_firm_category.csv")
...
write_csv(
  ...
  "shareclass_changes_passivefunds.csv"
  )
